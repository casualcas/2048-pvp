...
export function HomeScreen({ player, energy, maxEnergy, timeUntilRegen, onWatchAd, onBuy, adLoaded, noAds, onRemoveAds, onPlayPvP, onPlayBot, onPlaySolo, onLeaderboard, onSettings, onProfile }: Props) {
  ...
  const [activeTab, setActiveTab] = React.useState<'home' | 'leaders' | 'profile'>('home');
  const { t } = useLanguage();
  const { tasks, completedCount } = useDailyTasks();

  const handleLeaders = () => { setActiveTab('leaders'); onLeaderboard(); };
  const handleProfile = () => { setActiveTab('profile'); onProfile(); };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scroll}>
        ...
        {/* Daily tasks */}
        <DailyTasksWidget tasks={tasks} completedCount={completedCount} />
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: 8, paddingBottom: 4,
  },
  iconBtn: {
  ...
  title: { fontSize: 40, fontWeight: '900', color: theme.colors.text, letterSpacing: -1 },
  subtitle: { fontSize: 16, fontWeight: '900', color: theme.colors.accent1, letterSpacing: 6, marginTop: -6 },
  playerCard: {
    marginHorizontal: 20, marginBottom: 10, backgroundColor: theme.colors.bgCard,
    borderRadius: 16, padding: 14, borderWidth: 1, borderColor: theme.colors.border,
  },
  playerInfo: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 },
  ...
  statLabel: { fontSize: 10, color: theme.colors.text2, fontWeight: '600', letterSpacing: 1 },
  statDivider: { width: 1, backgroundColor: theme.colors.border },
  buttons: { paddingHorizontal: 20, gap: 10, marginBottom: 8 },
  pvpBtn: {
  ...
  btn2Title: { fontSize: 15, fontWeight: '900', color: theme.colors.text, marginBottom: 2 },
  btn2Sub: { fontSize: 11, color: theme.colors.text2 },
  shopBtn: {
    backgroundColor: theme.colors.bgCard, borderRadius: 16,
    paddingVertical: 12, alignItems: 'center',
    borderWidth: 2, borderColor: theme.colors.accent2,
  },
  shopBtnText: { fontSize: 15, fontWeight: '900', color: theme.colors.accent2, letterSpacing: 1 },
  tagline: { textAlign: 'center', color: theme.colors.text3, fontSize: 12, paddingBottom: 12, fontStyle: 'italic' },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.3)', justifyContent: 'flex-end' },
  modalCard: { backgroundColor: theme.colors.bg, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, gap: 10 },
